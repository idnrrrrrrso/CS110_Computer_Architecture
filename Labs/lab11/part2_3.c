...
void simd_matmul(void) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < p; j+=4) {
            ...
            for (int k = 0; k < 4; ++k) {
                __m128 va = _mm_set1_ps(a[i][k]); // a[i][k] into all elements of reg
                __m128 vb = _mm_loadu_ps(&b[k][j]); // b[k][j] vector
...
void loop_unroll_simd_matmul(void) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < p; j+=4) {
            ...
            __m128 va = _mm_set1_ps(a[i][0]);
            __m128 vb = _mm_loadu_ps(&b[0][j]);
            sum = _mm_add_ps(sum, _mm_mul_ps(va, vb));

            va = _mm_set1_ps(a[i][1]);
            vb = _mm_loadu_ps(&b[1][j]);
            sum = _mm_add_ps(sum, _mm_mul_ps(va, vb));

            va = _mm_set1_ps(a[i][2]);
            vb = _mm_loadu_ps(&b[2][j]);
            sum = _mm_add_ps(sum, _mm_mul_ps(va, vb));

            va = _mm_set1_ps(a[i][3]);
            vb = _mm_loadu_ps(&b[3][j]);
            sum = _mm_add_ps(sum, _mm_mul_ps(va, vb));
